...
import { fetchProducts, fetchProductById, allCategories } from '../mock/products'
...
export const useProductStore = create<ProductState>((set, get) => ({
  ...
  recommend: (id) => {
    ...
    const list = get().products
      .filter((p) => !id || (p.id !== id && p.category === target?.category))
...
function applyAll(products: Product[], state: ProductState, page: number, pageSize: number) {
